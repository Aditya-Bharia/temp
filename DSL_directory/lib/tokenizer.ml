...
let () =
  List.iter
    (fun (str, keyword) -> Hashtbl.add keyword_table str keyword)
    [ (* ── Control keywords ── *)
      "var", VAR
    ; "fn", FN
    ; "return", RETURN
    ...
    ; "minimize", MINIMIZE
    ; "regex_to_nfa", REGEX_TO_NFA
    ; "nfa_to_regex", NFA_TO_REGEX
    ; "dfa_to_regex", DFA_TO_REGEX
    ; (* ── Analysis functions ── *)
      "accepts", ACCEPTS
    ; "trace", TRACE
    ; "equivalent", EQUIVALENT
    ; "regex_equivalent", REGEX_EQUIVALENT
    ; "validate", VALIDATE
    ; "subset", SUBSET
    ; "is_empty", IS_EMPTY
    ; "is_finite", IS_FINITE
...
let rec read_string str chars line col =
  ...
  | '\\' :: '"' :: tail ->
    read_string (str ^ "\"") tail line (col + 2)
    (* escape sequence  "say \"hello\"" as say hello and not say \*)
  | '\n' :: tail ->
...
let rec read_identifier_or_keyword acc chars line col =
  match chars with
  | (('a' .. 'z' | 'A' .. 'Z' | '0' .. '9' | '_') as c) :: tail ->
    read_identifier_or_keyword (acc ^ String.make 1 c) tail line (col + 1)
...
let rec skip_block_comment chars line col =
  ...
  | [] ->
    failwith
      (Printf.sprintf "Lexer error at line %d, col %d :Block comment not closed" line col)
...
let rec tokenize chars line col =
  ...
  (* ── delimiters ── *)
  | '{' :: tail ->
    { kind = LEFT_CURL; text = "{"; lit_val = "null"; line; col }
    :: tokenize tail line (col + 1)
  | '}' :: tail ->
    { kind = RIGHT_CURL; text = "}"; lit_val = "null"; line; col }
    :: tokenize tail line (col + 1)
  | '(' :: tail ->
    { kind = LEFT_PAR; text = "("; lit_val = "null"; line; col }
    :: tokenize tail line (col + 1)
  | ')' :: tail ->
    { kind = RIGHT_PAR; text = ")"; lit_val = "null"; line; col }
    :: tokenize tail line (col + 1)
  | '[' :: tail ->
    { kind = LEFT_BRACKET; text = "["; lit_val = "null"; line; col }
    :: tokenize tail line (col + 1)
  | ']' :: tail ->
    { kind = RIGHT_BRACKET; text = "]"; lit_val = "null"; line; col }
    :: tokenize tail line (col + 1)
  | ',' :: tail ->
    { kind = COMMA; text = ","; lit_val = "null"; line; col }
    :: tokenize tail line (col + 1)
  | '.' :: tail ->
    { kind = DOT; text = "."; lit_val = "null"; line; col } :: tokenize tail line (col + 1)
  (* ── arithmetic operators ── *)
  | '+' :: tail ->
    { kind = PLUS; text = "+"; lit_val = "null"; line; col }
    :: tokenize tail line (col + 1)
  | '*' :: tail ->
    { kind = STAR; text = "*"; lit_val = "null"; line; col }
    :: tokenize tail line (col + 1)
  | '/' :: tail ->
    { kind = SLASH; text = "/"; lit_val = "null"; line; col }
    :: tokenize tail line (col + 1)
  (* ── multi-char operators ── *)
  | '-' :: '>' :: tail ->
    { kind = ARROW; text = "->"; lit_val = "null"; line; col }
    :: tokenize tail line (col + 2)
  | '-' :: tail ->
    { kind = MINUS; text = "-"; lit_val = "null"; line; col }
    :: tokenize tail line (col + 1)
  | '=' :: '=' :: tail ->
    { kind = EQ; text = "=="; lit_val = "null"; line; col } :: tokenize tail line (col + 2)
  | '=' :: tail ->
    { kind = ASSIGN; text = "="; lit_val = "null"; line; col }
    :: tokenize tail line (col + 1)
  | '!' :: '=' :: tail ->
    { kind = NEQ; text = "!="; lit_val = "null"; line; col }
    :: tokenize tail line (col + 2)
  | '<' :: '=' :: tail ->
    { kind = LEQ; text = "<="; lit_val = "null"; line; col }
    :: tokenize tail line (col + 2)
  | '<' :: tail ->
    { kind = LESS; text = "<"; lit_val = "null"; line; col }
    :: tokenize tail line (col + 1)
  | '>' :: '=' :: tail ->
    { kind = GEQ; text = ">="; lit_val = "null"; line; col }
    :: tokenize tail line (col + 2)
  | '>' :: tail ->
    { kind = MORE; text = ">"; lit_val = "null"; line; col }
    :: tokenize tail line (col + 1)
  (* ── string literal ── *)
  | '"' :: tail ->
    let old_line = line
  ...
  | ('0' .. '9' as d) :: tail1 ->
    let old_line = line
    and old_col = col in
    let num, tail2, new_line, new_col =
      read_number (String.make 1 d) tail1 line (col + 1)
    in
  ...
  | (('a' .. 'z' | 'A' .. 'Z' | '_') as ch) :: tail1 ->
    let old_line = line
    and old_col = col in
    let word, tail2, new_line, new_col =
      read_identifier_or_keyword (String.make 1 ch) tail1 line (col + 1)
    in
  ...
  | c :: _ ->
    failwith
      (Printf.sprintf
         "Lexer error at line %d, col %d: unexpected character '%c'"
         line
         col
         c)
;;
...
let rec print_tokens tok_list =
  match tok_list with
  | [] -> ""
  | { kind; text; lit_val; line; col } :: tail ->
    let kind_to_str =
      match kind with
      (* ── Control keywords ── *)
      | VAR -> "VAR"
