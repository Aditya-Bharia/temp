...
let rec tokenize chars line col =
  ...
  | '/' :: '*' :: tail1 ->
      ...
      tokenize tail2 new_line new_col
  (* ── delimiters ── *)
  | '{' :: tail -> { kind = LEFT_CURL;     text = "{"; lit_val = "null"; line; col } :: tokenize tail line (col+1)
  | '}' :: tail -> { kind = RIGHT_CURL;    text = "}"; lit_val = "null"; line; col } :: tokenize tail line (col+1)
  | '(' :: tail -> { kind = LEFT_PAR;      text = "("; lit_val = "null"; line; col } :: tokenize tail line (col+1)
  | ')' :: tail -> { kind = RIGHT_PAR;     text = ")"; lit_val = "null"; line; col } :: tokenize tail line (col+1)
  | '[' :: tail -> { kind = LEFT_BRACKET;  text = "["; lit_val = "null"; line; col } :: tokenize tail line (col+1)
  | ']' :: tail -> { kind = RIGHT_BRACKET; text = "]"; lit_val = "null"; line; col } :: tokenize tail line (col+1)
  | ',' :: tail -> { kind = COMMA;         text = ","; lit_val = "null"; line; col } :: tokenize tail line (col+1)
  | '.' :: tail -> { kind = DOT;           text = "."; lit_val = "null"; line; col } :: tokenize tail line (col+1)
  (* ── arithmetic operators ── *)
  | '+' :: tail -> { kind = PLUS;  text = "+"; lit_val = "null"; line; col } :: tokenize tail line (col+1)
  | '*' :: tail -> { kind = STAR;  text = "*"; lit_val = "null"; line; col } :: tokenize tail line (col+1)
  | '/' :: tail -> { kind = SLASH; text = "/"; lit_val = "null"; line; col } :: tokenize tail line (col+1)
  (* ── multi-char operators ── *)
  | '-' :: '>' :: tail -> { kind = ARROW;  text = "->"; lit_val = "null"; line; col } :: tokenize tail line (col+2)
  | '-' :: tail -> { kind = MINUS;  text = "-";  lit_val = "null"; line; col } :: tokenize tail line (col+1)
  | '=' :: '=' :: tail -> { kind = EQ;     text = "=="; lit_val = "null"; line; col } :: tokenize tail line (col+2)
  | '=' :: tail -> { kind = ASSIGN; text = "=";  lit_val = "null"; line; col } :: tokenize tail line (col+1)
  | '!' :: '=' :: tail -> { kind = NEQ;    text = "!="; lit_val = "null"; line; col } :: tokenize tail line (col+2)
  | '<' :: '=' :: tail -> { kind = LEQ;    text = "<="; lit_val = "null"; line; col } :: tokenize tail line (col+2)
  | '<' :: tail -> { kind = LESS;   text = "<";  lit_val = "null"; line; col } :: tokenize tail line (col+1)
  | '>' :: '=' :: tail -> { kind = GEQ;    text = ">="; lit_val = "null"; line; col } :: tokenize tail line (col+2)
